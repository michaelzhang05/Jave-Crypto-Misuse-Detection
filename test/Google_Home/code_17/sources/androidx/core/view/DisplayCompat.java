...
public final class DisplayCompat {
        ...
        public static ModeCompat[] getSupportedModes(@NonNull Context context, @NonNull Display display) {
            ...
            if (currentDisplaySizeFromWorkarounds != null && !physicalSizeEquals(mode, currentDisplaySizeFromWorkarounds)) {
                for (int i8 = 0; i8 < supportedModes.length; i8++) {
                    if (physicalSizeEquals(supportedModes[i8], mode)) {
                        modeCompat = new ModeCompat(supportedModes[i8], currentDisplaySizeFromWorkarounds);
                    } else {
                        modeCompat = new ModeCompat(supportedModes[i8], false);
                    }
                    modeCompatArr[i8] = modeCompat;
                }
            } else {
                for (int i9 = 0; i9 < supportedModes.length; i9++) {
                    modeCompatArr[i9] = new ModeCompat(supportedModes[i9], physicalSizeEquals(supportedModes[i9], mode));
                }
            }
    ...
    public static final class ModeCompat {
        ...
        @RequiresApi(23)
        ModeCompat(@NonNull Display.Mode mode, boolean z8) {
