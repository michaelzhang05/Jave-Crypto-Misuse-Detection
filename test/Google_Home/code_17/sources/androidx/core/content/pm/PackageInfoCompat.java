...
public final class PackageInfoCompat {
    ...
    public static class Api28Impl {
        ...
        @DoNotInline
        static boolean hasSigningCertificate(@NonNull PackageManager packageManager, @NonNull String str, @NonNull byte[] bArr, int i8) {
    ...
    private static byte[] computeSHA256Digest(byte[] bArr) {
        try {
            ...
            throw new RuntimeException("Device doesn't support SHA256 cert checking", e8);
        }
    }
    ...
    public static boolean hasSignatures(@NonNull PackageManager packageManager, @NonNull String str, @NonNull @Size(min = 1) Map<byte[], Integer> map, boolean z8) throws PackageManager.NameNotFoundException {
        byte[][] bArr;
        if (map.isEmpty()) {
            return false;
        }
        ...
        if (signatures.size() != 0 && map.size() <= signatures.size() && (!z8 || map.size() == signatures.size())) {
