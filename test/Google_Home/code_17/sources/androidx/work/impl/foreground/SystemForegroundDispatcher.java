...
public class SystemForegroundDispatcher implements OnConstraintsStateChangedListener, ExecutionListener {
    ...
    final Map<WorkGenerationalId, InterfaceC3390x0> mTrackedWorkSpecs;
    ...
    public void onDestroy() {
        ...
        synchronized (this.mLock) {
            try {
                Iterator<InterfaceC3390x0> it = this.mTrackedWorkSpecs.values().iterator();
                while (it.hasNext()) {
                    it.next().cancel(null);
                }
            } catch (Throwable th) {
        ...
        synchronized (this.mLock) {
            try {
                if (this.mWorkSpecById.remove(workGenerationalId) != null) {
                    interfaceC3390x0 = this.mTrackedWorkSpecs.remove(workGenerationalId);
                } else {
                    interfaceC3390x0 = null;
                }
                if (interfaceC3390x0 != null) {
                    interfaceC3390x0.cancel(null);
                }
            } catch (Throwable th) {
                throw th;
            }
