...
public class TransitionManager {
    ...
    private static void changeScene(Scene scene, Transition transition) {
        ...
        if (!sPendingTransitions.contains(sceneRoot)) {
            ...
            Transition mo5477clone = transition.mo5477clone();
            if (currentScene != null && currentScene.isCreatedFromLayoutResource()) {
                mo5477clone.setCanRemoveViews(true);
            }
            sceneChangeSetup(sceneRoot, mo5477clone);
            scene.enter();
            sceneChangeRunTransition(sceneRoot, mo5477clone);
        }
    }
    ...
    public static TransitionSeekController controlDelayedTransition(@NonNull ViewGroup viewGroup, @NonNull Transition transition) {
        ...
        if (transition.isSeekingSupported()) {
            ...
            transitionSet.addTransition(mo5477clone);
    ...
    public static void beginDelayedTransition(@NonNull ViewGroup viewGroup, @Nullable Transition transition) {
        ...
        Transition mo5477clone = transition.mo5477clone();
        sceneChangeSetup(viewGroup, mo5477clone);
        Scene.setCurrentScene(viewGroup, null);
        sceneChangeRunTransition(viewGroup, mo5477clone);
    }
