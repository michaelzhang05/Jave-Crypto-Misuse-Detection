...
public final class DiskLruCache implements Closeable, Flushable {
            ...
            if (this.entry.currentEditor == this) {
                ...
                while (true) {
                    ...
                    if (i8 < diskLruCache.valueCount) {
                        try {
                            diskLruCache.fileSystem.delete(this.entry.dirtyFiles[i8]);
                        } catch (IOException unused) {
                        }
                        i8++;
                    } else {
            ...
            synchronized (DiskLruCache.this) {
                try {
                    if (!this.done) {
                        ...
                        if (!entry.readable) {
                            this.written[i8] = true;
                        }
                        try {
                            return new FaultHidingSink(DiskLruCache.this.fileSystem.sink(entry.dirtyFiles[i8])) { // from class: com.mbridge.msdk.thrid.okhttp.internal.cache.DiskLruCache.Editor.1
            ...
            synchronized (DiskLruCache.this) {
                try {
                    if (!this.done) {
                        ...
                        try {
                            return DiskLruCache.this.fileSystem.source(entry.cleanFiles[i8]);
                        } catch (FileNotFoundException unused) {
                            return null;
                        }
                    }
    ...
    public final class Entry {
        ...
        Entry(String str) {
            this.key = str;
            int i8 = DiskLruCache.this.valueCount;
            this.lengths = new long[i8];
            this.cleanFiles = new File[i8];
            this.dirtyFiles = new File[i8];
        ...
        void setLengths(String[] strArr) throws IOException {
            if (strArr.length == DiskLruCache.this.valueCount) {
                for (int i8 = 0; i8 < strArr.length; i8++) {
                    try {
                        this.lengths[i8] = Long.parseLong(strArr[i8]);
                ...
                while (true) {
                    try {
                        ...
                        if (i9 < diskLruCache.valueCount) {
                            sourceArr[i9] = diskLruCache.fileSystem.source(this.cleanFiles[i9]);
                            i9++;
                        } else {
                        ...
                        while (true) {
                            DiskLruCache diskLruCache2 = DiskLruCache.this;
                            if (i8 < diskLruCache2.valueCount && (source = sourceArr[i8]) != null) {
    ...
    private void processJournal() throws IOException {
        ...
        while (it.hasNext()) {
            ...
            if (next.currentEditor == null) {
                ...
                while (i8 < this.valueCount) {
                    this.fileSystem.delete(next.cleanFiles[i8]);
                    this.fileSystem.delete(next.dirtyFiles[i8]);
                    i8++;
                }
                it.remove();
            }
    ...
    synchronized void completeEdit(Editor editor, boolean z8) throws IOException {
        Entry entry = editor.entry;
        if (entry.currentEditor == editor) {
            if (z8 && !entry.readable) {
                for (int i8 = 0; i8 < this.valueCount; i8++) {
                    if (editor.written[i8]) {
                        if (!this.fileSystem.exists(entry.dirtyFiles[i8])) {
                            editor.abort();
                            return;
                        }
                    } else {
            ...
            for (int i9 = 0; i9 < this.valueCount; i9++) {
                File file = entry.dirtyFiles[i9];
                if (z8) {
                    if (this.fileSystem.exists(file)) {
                        File file2 = entry.cleanFiles[i9];
                        this.fileSystem.rename(file, file2);
                        long j8 = entry.lengths[i9];
                        long size = this.fileSystem.size(file2);
                        entry.lengths[i9] = size;
        ...
        try {
            ...
            if (this.fileSystem.exists(this.journalFile)) {
                try {
                    ...
                    Platform.get().log(5, "DiskLruCache " + this.directory + " is corrupt: " + e8.getMessage() + ", removing", e8);
                    try {
                        delete();
                        this.closed = false;
                    } catch (Throwable th) {
    ...
    boolean removeEntry(Entry entry) throws IOException {
        ...
        for (int i8 = 0; i8 < this.valueCount; i8++) {
            this.fileSystem.delete(entry.cleanFiles[i8]);
            long j8 = this.size;
            long[] jArr = entry.lengths;
            this.size = j8 - jArr[i8];
            jArr[i8] = 0;
        }
        this.redundantOpCount++;
    ...
    synchronized Editor edit(String str, long j8) throws IOException {
        initialize();
        checkNotClosed();
        validateKey(str);
        Entry entry = this.lruEntries.get(str);
        if (j8 != -1 && (entry == null || entry.sequenceNumber != j8)) {
            return null;
        }
