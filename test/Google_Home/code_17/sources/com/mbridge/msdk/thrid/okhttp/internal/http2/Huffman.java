...
class Huffman {
    ...
    private void addCode(int i8, int i9, byte b8) {
        Node node = new Node(i8, b8);
        ...
        while (b8 > 8) {
            ...
            if (nodeArr != null) {
                if (nodeArr[i10] == null) {
                    nodeArr[i10] = new Node();
                }
                node2 = node2.children[i10];
            } else {
    ...
    public byte[] decode(byte[] bArr) {
        ...
        while (i9 > 0) {
            ...
            if (node2.children != null || node2.terminalBits > i9) {
                break;
            }
            byteArrayOutputStream.write(node2.symbol);
            i9 -= node2.terminalBits;
            node = this.root;
        }
    ...
    public void encode(ByteString byteString, BufferedSink bufferedSink) throws IOException {
        ...
        for (int i9 = 0; i9 < byteString.size(); i9++) {
            ...
            int i11 = CODES[i10];
            byte b8 = CODE_LENGTHS[i10];
            j8 = (j8 << b8) | i11;
            i8 += b8;
            while (i8 >= 8) {
                i8 -= 8;
                bufferedSink.writeByte((int) (j8 >> i8));
            }
        }
        if (i8 > 0) {
            bufferedSink.writeByte((int) ((j8 << (8 - i8)) | (255 >>> i8)));
        }
    }
    ...
    public int encodedLength(ByteString byteString) {
        ...
        for (int i8 = 0; i8 < byteString.size(); i8++) {
            j8 += CODE_LENGTHS[byteString.getByte(i8) & 255];
        }
        return (int) ((j8 + 7) >> 3);
    }
