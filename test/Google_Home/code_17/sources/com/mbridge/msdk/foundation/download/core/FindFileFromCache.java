...
class FindFileFromCache implements IDownloadTask {
    ...
    private boolean checkDownloadReadyRate(long j8) {
        if (Utils.getDownloadRate(this._downloadModel.getTotalBytes(), j8) >= this._downloadMessage.getDownloadRate()) {
            return true;
        }
        return false;
    }
    ...
    private void resumeDownloadFromFile(DownloadResponse downloadResponse, long j8) {
        this._request.setTotalBytes(this._downloadModel.getTotalBytes());
        this._request.setDownloadedBytes(j8);
        GlobalComponent.getInstance().getDatabaseHelper().update(DownloadModel.create(this._downloadModel.getDownloadId(), this._downloadModel.getDownloadUrl(), this._downloadModel.getResourceUrl(), this._downloadModel.getEtag(), this._downloadModel.getSaveDirectorPath(), this._downloadModel.getSaveFileName(), this._downloadModel.getTotalBytes(), j8, this._downloadModel.getDownloadRate(), this._downloadModel.getUsageCounter(), this._downloadModel.getDownloadResourceType()), this._request.getCacheDirectoryPath());
        downloadResponse.setSuccessful(checkDownloadReadyRate(j8));
    }
