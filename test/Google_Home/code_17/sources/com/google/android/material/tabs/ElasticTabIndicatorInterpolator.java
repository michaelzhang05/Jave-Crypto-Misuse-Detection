...
public class ElasticTabIndicatorInterpolator extends TabIndicatorInterpolator {
    private static float accInterp(@FloatRange(from = 0.0d, to = 1.0d) float f8) {
        return (float) (1.0d - Math.cos((f8 * 3.141592653589793d) / 2.0d));
    }

    private static float decInterp(@FloatRange(from = 0.0d, to = 1.0d) float f8) {
    ...
    public void updateIndicatorForOffset(TabLayout tabLayout, View view, View view2, float f8, @NonNull Drawable drawable) {
        ...
        if (calculateIndicatorWidthForTab.left < calculateIndicatorWidthForTab2.left) {
            decInterp = accInterp(f8);
            accInterp = decInterp(f8);
        } else {
            decInterp = decInterp(f8);
            accInterp = accInterp(f8);
        }
