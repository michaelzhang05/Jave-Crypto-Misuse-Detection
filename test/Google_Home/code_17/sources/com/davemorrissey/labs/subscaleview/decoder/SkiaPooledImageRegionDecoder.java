...
public class SkiaPooledImageRegionDecoder implements ImageRegionDecoder {
    ...
    public void initialiseDecoder() throws Exception {
        ...
        if (uri.startsWith(RESOURCE_PREFIX)) {
            ...
            try {
                j8 = this.context.getResources().openRawResourceFd(i8).getLength();
            } catch (Exception unused2) {
            }
            bitmapRegionDecoder = BitmapRegionDecoder.newInstance(this.context.getResources().openRawResource(i8), false);
        } else if (uri.startsWith(ASSET_PREFIX)) {
            String substring = uri.substring(22);
            try {
                j8 = this.context.getAssets().openFd(substring).getLength();
                    ...
                    while (SkiaPooledImageRegionDecoder.this.decoderPool != null) {
                        ...
                        if (skiaPooledImageRegionDecoder.allowAdditionalDecoder(skiaPooledImageRegionDecoder.decoderPool.size(), SkiaPooledImageRegionDecoder.this.fileLength)) {
                            try {
                                ...
                                SkiaPooledImageRegionDecoder.this.debug("Failed to start decoder: " + e8.getMessage());
                            }
                        } else {
                            return;
                        }
    ...
    protected boolean allowAdditionalDecoder(int i8, long j8) {
        if (i8 >= 4) {
        ...
        debug("Additional decoder allowed, current count is " + i8 + ", estimated native memory " + (j9 / 1048576) + "Mb");
        return true;
    }
