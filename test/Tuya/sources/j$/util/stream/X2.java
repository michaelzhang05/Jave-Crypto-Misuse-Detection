...
public class X2 extends Z2 implements LongConsumer {
    ...
    public final String toString() {
        long[] jArr = (long[]) d();
        if (jArr.length < 200) {
            return String.format("%s[length=%d, chunks=%d]%s", getClass().getSimpleName(), Integer.valueOf(jArr.length), Integer.valueOf(this.f32500c), Arrays.toString(jArr));
        }
        return String.format("%s[length=%d, chunks=%d]%s...", getClass().getSimpleName(), Integer.valueOf(jArr.length), Integer.valueOf(this.f32500c), Arrays.toString(Arrays.copyOf(jArr, 200)));
    }
    ...
    public j$.util.c0 spliterator() {
        return new W2(this, 0, this.f32500c, 0, this.f32499b);
    }
}
